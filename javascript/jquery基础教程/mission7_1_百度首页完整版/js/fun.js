...
$(window).scroll(function () {
    ...
    //获取搜索框容器的ID
    var flowSearch = $("#researchWrapper");
    //获取变化后搜索框左边Logo的ID
    var searchImg = $("#researchImg");
    //获取变化前搜索框上部Logo的ID
    var mainLogo = $("#headLogoImg");
    //获取页首导航栏的ID
    var navBar = $("NavWrapper");
...
//对页面操作开始
var isChange = "0";
var BGmenu = document.getElementById('changeStyleMenu')
...
//选择背景图片类型
var curImgUlId = "#ImgPage1_remen";
var changeImgUlId;
var NumberOfImgPage = "1";
var KindOfImgPage = {};
$(curImgUlId).show();
...
BGmenu.onmouseover = function (ev) {
    ...
    for (var i = 0, l = BCpic.length; i < l; i++) {
        if (BCpic[i] == target) {
            var SRC = "img/changeColorImg/" + KindOfImgPage[2] + "_Page" + NumberOfImgPage + "/p" + (i % 12 + 1) + '.jpg';
...
BGmenu.onclick = function (ev) {
    ...
    for (var i = 0, l = BCpic.length; i < l; i++) {
        if (BCpic[i] == target) {
            var SRC = "img/changeColorImg/" + KindOfImgPage[2] + "_Page" + KindOfImgPage[0] + "/p" + (i % 12 + 1) + '.jpg';
            ...
            //设置Cookie
            //            Cookies.set('BackgroundImg', SRC);
            //            Cookies.set('IsChange', '1');
            //设置localStorage
...
var TmpButtomNavButtonId = "#buttomNavWrapper_Recommend";
//加载页面时显示第二个选项"推荐"对应的内容
$("#buttomMenu_Recommend").css("display", "block");
//采用事件代理对li进行操作
$("#buttomNavWrapperLeft").delegate("li", "click", function () {
    //获取当前下部页面(需要隐藏)的ID
    var TmpMenuId = '#buttomMenu_' + TmpButtomNavButtonId.split('_')[1];
    //隐藏未选择前的页面
    $(TmpMenuId).css("display", "none");
    //获取导航栏选择项对应的页面的ID
    var CurMenuId = '#buttomMenu_' + $(this).attr("id").split('_')[1];
    //改变其css属性,使其显示
...
//获取底部导航栏的栏目
var buttomNavChoice = $("#buttomNavWrapperLeft").children("ul").children("li");
//var IsButtomNavChoice[buttomNavChoice.length];
//for(var i=0, l = buttomNavChoice.length; i<l; i++){
//}
...
$("#buttomMenu_setting_UnChosenMenu").delegate("li", "click", function () {
    ...
    var chosenText = $(this).text(), //获取已订阅项目的列表
        unChosenText = $("#buttomMenu_setting_ChosenMenu").children("ul").children("li");
    //在已订阅列表中寻找对应的项目
    for (var i = 0, l = unChosenText.length; i < l; i++) {
        //若符合条件,则执行操作
        if (unChosenText[i].innerHTML === chosenText) {
            $(unChosenText[i]).show();
            $(this).hide();
            for (var j = 0, l2 = buttomNavChoice.length; j < l2; j++) {
...
$("#buttomMenu_setting_ChosenMenu").delegate("li", "click", function () {
    ...
    var UnchosenText = $(this).text(), //获取为订阅项目的列表
        ChosenText = $("#buttomMenu_setting_UnChosenMenu").children("ul").children("li");
    //在未订阅列表中寻找对应的项目
    for (var i = 0, l = ChosenText.length; i < l; i++) {
        //若符合条件,则执行操作
